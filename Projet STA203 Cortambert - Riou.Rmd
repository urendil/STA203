...
reconstruct <- function(res,nr,Xm,Xsd){
  ...
  for (i in 1:nr){
    plot(res,axes = i:i+1,choix = "var")
    plot(Xsd,col="blue",add=TRUE)
    plot(Xm,col="blue",add=TRUE)
  }
}
```
...
plot(ridge_fit$a0,main="évolution des coefficients de l'intercept",ylab="Valeur de l'intercept")

abline(h=ridge_fit$a0[100],col="red",lty=2)
...
```{r}
theta_chap <- function(k){ (t(xtrain)*xtrain+k*diag(1,nrow=n))^(-1)*t(xtrain)*ytrain
}
k <- seq(0,1,by = 0.01)
plot(unlist(theta_chap(k)),main="Représentation de l'évolution de l'estimateur theta \n en fonction de kappa",ylab="theta_chap",xlab="kappa",type="o",xlim=c(0,100))
```
