...
L'ACP ne permet pas de réduire les dimensions en suivant la règle du "coude" sauf si nous voulons garder qu'un axe et perdre 69% de l'information.
...
for (i in y){
  ...
  text(which.max(x.pca[i,])-100,1,which.max(x.pca[i,]),col="red")
}
```
...
```
De même, nous pouvons comparer les fréquences pour lesquelles ce maximum est atteint. La tendance confirme que le maximum est atteint pour les plus hautes fréquences (Nous pouvions nous en douter graphiquement). Cependant, ces fréquences ne sont pas identiques.
```{r}
...
```{r}
theta_chap <- function(k){ (t(x)*x+k*diag(1,nrow=p))^(-1)*t(x)*ytrain
}
k <- seq(1,10,by=0.01)
plot(unlist(theta_chap(k)),main="Régression Ridge",ylab="theta_chap",xlab="kappa",type="l",xlim=c(1,10),col="blue")
```
...
set.seed(1)
B <- 3
folds <- cvsegments(nrow(x.pca),B, type="random")
```
...
X = as.data.frame(model.matrix(lmtot))

for(b in 1:B){
  subsetb=unlist(folds[[b]])
  best.fit=regsubsets(ytrain~.,data=X[-subsetb,],really.big = TRUE)
...
plot(lm.ridge(ytrain~.,data = x.pca,lambda=seq(0,100,0.1)))
```


```{r}
gridRidge <- 10^seq(0,2,0.01)
plot(lm.ridge(ytrain~.,data = x.pca,lambda=gridRidge),main="",xlim=c(0,20))
abline(h=0)
```
