...
dim(xtest)
```
Et finalement rajoutons un peu de bruit à notre modèle.
...
for (i in c(1,0.7,0.5,0.2,0.1,0.01))
{
  eps <- rnorm(700,mean=0,sd=sigma2)
  x.train <- xtrain + eps
  plot(seq(1,700),x.train[1,],main=c("sigma =",i),type="l")
}
```
Afin que notre modèle ne soit pas trop bruité et que ce soit réaliste nous choisissons unevaleur de $\sigma$ de 0.1.
...
plot(x.sample,
     main="écart-type des échantillons X1, X10, X50, X100, X500 et X700",
     ylab="écart-type",type="o",col="red")
```
Le graphique nous montre effectivement une augmentation de la variance avec la fréquence.
...
corrplot(C)
```
Nous constatons que la corrélation des variables entre elles diminuent avec la fréquence. La corrélation entre $X1$ et $X50$ est plus importante qu'entre les variables $X1$ et $X700$. Nous pouvons suppposer dès maintenant que l'analyse exploratoire va être compliquée considérant la corrélation des variables. Tentons quand même une analyse par composantes principales pour confirmer la forte corrélation des variables.
...
valp <- eigen(cor(x.train))$values[1:6]
```

Le graphique montre que les 2 voire 3 premières variables contiennent toute l'information. Par conséquent, les valeurs propres des variables suivantes tendent rapidement vers 0. Si nous voulons faire une analyse plus poussée, il faudra considérer de pénaliser les paramètres (Ridge ou LASSO).

Réalisons maintenant une ACP sur les 5 premiers axes principaux en faisant apparaître le $cos2$. Le sixième axe n'est pas représentable. Voici les graphiques obtenus:
...
fviz_pca_biplot(pca.res,axes = c(5,6),label="ind",
                col.var = "cos2",select.ind = list(name=c("1","2","3","4","5","39")),
                select.var = list(cos2 = 0.1))
fviz_pca_biplot(pca.res,axes = c(6,1),label="ind",
                col.var = "cos2",select.ind = list(name=c("1","2","3","4","5","39")),
                select.var = list(cos2 = 0.1))
```
...
```{r}
y <- c(1:5,39)
x <- scale(x.train[y,], scale = TRUE , center = TRUE)
C.sample <- cor(x)
corrplot(C.sample)
```
...
par(mfrow=c(2,3))
for (i in y){
  glmModel <- glm(ytrain[i]~.,data = x.train[i,])
  Preds <- predict(glmModel,type="response")
  plot(seq(1:700),xtrain[i,],
       main = paste("RMSE = ",round(c(RMSE(ytrain[i],Preds)),2),"\n MAE = ",round(c(MAE(ytrain[i],Preds)),4)),
       type = "l",
       xlab = "Fréquences",ylab=paste("Cookie ",i," max = ",round(max(x.train[i,]),3)))
  abline(v = which.max(x.train[i,]),col="red",lty=2)
  abline(h = max(x.train[i,]),col="red",lty=2)
  text(which.max(x.train[i,]),1,max(x.train[i,]),col="red")
}
```
Il est difficile de comparer les courbes entre elles car la forme générale semble identique. Prenons un critère (le maximum) afin de comparer ces courbes. Nous constatons que le maximum des courbes est différent:

```{r}
round(apply(x.train[y,],1,max),3)
```
De même, nous pouvons comparer les fréquences pour lesquelles ce maximum est atteint. La tendance confirme que le maximum est atteint pour les plus hautes fréquences (Nous pouvions nous en douter grpahiquement). Cependant, ces fréquences ne sont pas identiques.
```{r}
round(apply(x.train[y,],1,which.max),3)
```
Réappliquons cette méthode sur la variable $X24$ afin de déterminer l'erreur des moindres-carrés (RMSE) et 

```{r}
par(mfrow=c(1,1))
indice <- 24
glmModel <- glm(ytrain[indice]~.,data=x.train[indice,])
Preds <- predict(glmModel,type="response")
plot(seq(1:700),x.train[indice,],main = paste("RMSE = ",round(RMSE(x.train[indice,],Preds),2),"\n MAE = ",round(MAE(x.train[indice,],Preds),4)),type = "l",xlab = "Fréquences",ylab=paste("Cookie ",indice))
abline(v = which.max(x.train[indice,]),col="red",lty=2)
abline(h = max(x.train[indice,]),col="red",lty=2)
text(500,max(x.train[indice,]),max(x.train[indice,]),col="red")
```
...
plot(ridge_fit$lambda[y],xlim=c(0,10),type="o")

```


```{r}
theta_chap <- function(k){ (t(x.train)*x.train+k*diag(1,nrow=p))^(-1)*t(x.train)*ytrain
}
k <- seq(0,10,by = 1)
...
x.scale <- as.data.frame(cbind(y.scale,scale(x.train , scale=TRUE , center=TRUE) ))
...
text(which.max(ridge_fit.scale$a0),0,which.max(ridge_fit.scale$a0),col="red")
```


## Centrage de X et non de Y
```{r}
x.scale <- as.data.frame(cbind(ytrain,scale(x.train , scale=TRUE , center=TRUE) ))
...
```{r}
y.scale <- scale(ytrain, scale = TRUE , center = TRUE )
x.scale <- as.data.frame(cbind(y.scale,x.train))
xRidge_train.scale <- model.matrix(y.scale~., data=x.scale)
ridge_fit.scale <- glmnet(xRidge_train.scale, y.scale,alpha = 0, lambda=gridRidge)
plot(ridge_fit.scale$a0,main="évolution des coefficients de l'intercept",ylab="Valeur de l'intercept",type="l")
...
text(which.max(ridge_fit.scale$a0),-5,which.max(ridge_fit.scale$a0),col="red")
```
...
library(pls)
set.seed(1)
cvseg <- cvsegments(nrow(x.train),k=4,type="random")
```
...
library(MASS)
par(mfrow=c(1,2))
plot(lm.ridge(ytrain~.,data = x.train,lambda=seq(0,1,0.01)))
plot(lm.ridge(ytrain~.,data = x.train,lambda=seq(0,0.2,0.001)))
```


```{r}
plot(lm.ridge(ytrain~.,data = x.train,lambda=gridRidge),xlim=c(0,20))
```
...
plot(seq(0,0.5,length = 1000), res$press,main = "PRESS",xlab="nombre d'échantillons",ylab="Predicted Sum of Squares",type="l")
```
```{r}
kappaet <- res$kappaet
x.ridge <- lm.ridge(ytrain~.,data = x.train,lambda = kappaet)
coeff <- coef(x.ridge)
```
...
plot(fit.rid,cook.app[,1],main="Y_pred en fonction de ytrain",xlab="Valeurs prédites",ylab = "ytrain")
abline(a = 0 , b = 1,col="red")
res.rid <- fit.rid-cook.app[,1]
plot.res(fit.rid,res.rid,titre="Résidus")
#text(fit.rid,res.rid,round(res.rid,2))
```
Nous voyons sur la figure de gauche que l'estimation de $ytrain$ est bonne car les points sont tous (ou presque) sur la droite $y=x$.
```{r}
