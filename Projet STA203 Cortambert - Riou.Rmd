...
vectp <- eigen(cor(xtrain))$vectors
valp <- eigen(cor(xtrain))$values[1:6]
```
...
Afin de gagner en lisibilité, selectionnons quelques individus (1,2,3,4,5 et 39) et quelques variables (celles dont le cos2 est important). Voici les graphiques obtenus:
...
y <- c(1:5,39)
for (i in y){
  glmModel <- glm(ytrain[i]~.,data = xtrain[i,])
  Preds <- predict(glmModel,type="response")
  plot(seq(1:700),xtrain[i,],
       main = paste("RMSE = ",round(c(RMSE(ytrain[i],Preds)),2),"\n MAE = ",round(c(MAE(ytrain[i],Preds)),4)),
       type = "l",
       xlab = "Fréquences",ylab=paste("Cookie ",i," max = ",round(max(xtrain[i,]),3)))
  abline(v = which.max(xtrain[i,]),col="red",lty=2)
  abline(h = max(xtrain[i,]),col="red",lty=2)
  text(which.max(xtrain[i,]),1,max(xtrain[i,]),col="red")
}
```
...
```{r}
round(apply(xtrain[y,],1,which.max),3)
```
...
par(mfrow=c(1,1))
indice <- 24
glmModel <- glm(ytrain[indice]~.,data=xtrain[indice,])
Preds <- predict(glmModel,type="response")
plot(seq(1:700),xtrain[indice,],main = paste("RMSE = ",round(c(RMSE(xtrain[indice,],Preds)),2),"\n MAE = ",round(c(MAE(xtrain[indice,],Preds)),4)),type = "l",xlab = "Fréquences",ylab=paste("Cookie ",indice))
abline(v = which.max(xtrain[indice,]),col="red",lty=2)
abline(h = max(xtrain[indice,]),col="red",lty=2)
text(500,max(xtrain[indice,]),max(xtrain[indice,]),col="red")
```
...
```{r}
plot(lm.ridge(ytrain~.,data = xtrain,lambda=gridRidge),xlim=c(0,20))
```
